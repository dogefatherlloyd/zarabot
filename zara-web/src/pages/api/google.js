  ...
  try {
    ...
    const googleRes = await fetch(
      `https://customsearch.googleapis.com/customsearch/v1?key=${
        googleAPIKey ? googleAPIKey : process.env.NEXT_PUBLIC_GOOGLE_API_KEY
      }&cx=${
        googleCSEId ? googleCSEId : process.env.NEXT_PUBLIC_GOOGLE_CSE_ID
      }&q=${query}&num=5`
    );

    const googleData = await googleRes.json();

    const sources = googleData.items ? googleData.items.map(item => ({
      title: item.title,
      link: item.link,
    ...
    const sourcesWithText = await Promise.all(
      sources.map(async source => {
        try {
          ...
          const res = await Promise.race([fetch(source.link), timeoutPromise]);
          ...
          if (res) {
            ...
            if (parsed) {
              let sourceText = parsed.textContent ? cleanSourceText(parsed.textContent) : "";
    ...
    const answerMessage = { role: "user", content: answerPrompt }
